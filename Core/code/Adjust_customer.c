...
extern void MSharpAdjLR(BYTE LR);
...
void fuck_you_made()
{
    ...
    unsigned char xxData[47]={0,   250, 9,   250, 20, 250,  250, 20,  250, 250,
	                          20,  250, 250, 20,  250, 250, 250, 250, 250, 250,
    ...
    CScalerSetByte(_P7_AUTO_SHP_ADDR_B8, 0x00);
    CScalerWrite(_P7_AUTO_SHP_DATA_B9, 47, xxData, _NON_AUTOINC);
    CScalerSetByte(_P7_AUTO_SHP_ADDR_B8, 0x00); 
}
...
// Output Value : None
//--------------------------------------------------
void CAdjustBrightness(void)//亮度
{
	pData[0] =(0x65 + stConBriData.Brightness - 50);//0x65;//(0x80 + stConBriData.Brightness - 60);
	pData[1] =(0x63 + stConBriData.Brightness - 50);//0x63;// (0x80 + stConBriData.Brightness - 60);
	pData[2] =(0x63 + stConBriData.Brightness - 50);//0x63;// (0x80 + stConBriData.Brightness - 60);
	CAdjustSetBrightness();//pData[0-2]=0x76.0x76.0x76
    CAdjustBacklight();
}
...
//	pData[2]=0x85;
	...
	CAdjustSetContrast();//设置函数
//	for(cnt=0;cnt<125;cnt++)CAdjustSharpnessForScaleUp();
//	CAdjustYpbprhue(0xa0);
}
 ...
 void CVideoSetSaturation(BYTE ucValue)
{
	...
	ucSaturation = 140;//GET_VDC_CHROMA();
	if(ucValue > 50)
    {
...
{
	if(_GET_INPUT_SOURCE() == _SOURCE_VGA )//|| _GET_INPUT_SOURCE() == _SOURCE_HDMI)
    {
		CScalerPageSelect(_PAGE7);
	...
	else if(0)//(_GET_INPUT_SOURCE() == _SOURCE_DVI || _GET_INPUT_SOURCE() == _SOURCE_HDMI)
    {
	...
	if(0)//(_GET_INPUT_SOURCE()==_SOURCE_DVI || (_GET_INPUT_SOURCE() == _SOURCE_HDMI && !CHdmiFormatDetect()))//HDMI Source & input DVI timing
		CScalerSetBit(_P7_PEAKING_ENABLE_A9, ~_BIT3, 0x00);		// disable Peaking and Coring
	else
#endif
    {
