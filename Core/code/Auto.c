...
BYTE CAutoDoAutoConfig(void)
{
    ...
    result  = CAutoDoAutoPosition();//自动检测起始位置
	
	//user setting	
	if(stModeInfo.ModeCurr == _MODE_720x400_70HZ ||
		stModeInfo.ModeCurr == _MODE_720x400_85HZ  ||
		stModeInfo.ModeCurr == _MODE_640x400_70HZ)//设置确定的输入调整
	{
		CAdjustAdcClock(800, 2);//压缩率
		CAutoDoAutoPosition();//自动检测起始位置
		stModeUserData.Phase = 40;//0-63
		CAdjustPhase(stModeUserData.Phase);
	}
	else if(stModeInfo.ModeCurr == _MODE_1440x480i_60HZ)//优化cga信号自动调整
	{
		//stModeUserData.HPosition = stModeInfo.IHStartPos-66;
		//write your code
		CAdjustAdcClock(2050, 2);//压缩率
		CAutoDoAutoPosition();//自动检测起始位置
		stModeUserData.Phase = 40;//0-63（没有什么变化）
		CAdjustPhase(stModeUserData.Phase);
		CTimerDelayXms(100);
		if((stModeInfo.IHStartPos-66)<270 && (stModeInfo.IHStartPos-66)>264)
			stModeUserData.HPosition = stModeInfo.IHStartPos-66;
		else if(stModeInfo.IHStartPos-66<=264 || (stModeInfo.IHStartPos-66)>270)
		stModeUserData.HPosition = 285;//stModeInfo.IHStartPos-60;
		else
		stModeUserData.HPosition = stModeInfo.IHStartPos-66;
		CAdjustHPosition();
	}
/*-----------------修改位置---------------*/
	else if(stModeInfo.ModeCurr == _MODE_640x480_60HZ)//在这里添加输入的分辨率信号
	{
		//在这里添加输入信号的水平点数调整，具体修改第一个参数
		CAdjustAdcClock(900, 2);//压缩//915,2
		//自动检测起始位置
		CAutoDoAutoPosition();
		//0-63（没有什么变化）
		stModeUserData.Phase = 40;
		//修改输入相位参数
		CAdjustPhase(stModeUserData.Phase);
	} 
	//system default
    ...
    {
        result  = CAutoDoAutoClock();//自动时钟
        if(result == _ERROR_SUCCESS)
        {
            result  = CAutoDoAutoPhase();//自动相位
            if((result == _ERROR_SUCCESS) || (result == _ERROR_PHASE))
            {
                if(result == _ERROR_PHASE)//1.改变相位
                {
                    stModeUserData.Phase = phasetemp;
                    CAdjustPhase(stModeUserData.Phase);
                }
                result  = CAutoDoAutoPosition();//再次检测相位
                if(result != _ERROR_SUCCESS)//相位设置不成功，设置显示位置
                {
