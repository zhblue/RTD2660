...
//--------------------------------------------------
BYTE CModeGetScaleSetting(void)//获取缩放
{
	BYTE 	option = 0;
#if(_HDMI_SUPPORT == _ON)
	...
	if((_GET_INPUT_SOURCE() ==_SOURCE_DVI || _GET_INPUT_SOURCE() ==_SOURCE_HDMI) && CHdmiFormatDetect())//HDMI only	
	{
		...
		CScalerPageSelect(_PAGE2);
		CScalerGetDataPortByte(_P2_HDMI_PSAP_CD, 0x07, 1, pData, _NON_AUTOINC);
		if(1)//pData[0] == 20) // EricLee for GIEC DVD HDMI full display
        {//1920x1080ix50Hz
...
#if(_YPBPR_SUPPORT && _YPBPR_NEW_SYNC_DETECT == _ON)		// Set SOG0,SOG1 sync level to default 320mv
    if (_GET_INPUT_SOURCE() == _SOURCE_YPBPR)
    {
