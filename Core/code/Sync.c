...
// Eric 0618
	...
	{	
	case _SEARCH_STATE:
		if (CSyncHandler())//同步测试
		{
			// Source and sync OK
			if (CModeDetect())//模式设定 
			{
				if (stModeInfo.ModeCurr == _MODE_NOSIGNAL)//无信号
				{
#if(_HDMI_HOT_PLUG_OPTION)
                    bDVIDetect = 0;
#endif
				...
				else if (stModeInfo.ModeCurr == _MODE_NOSUPPORT)//不支持的信号
				{
#if(_HDMI_HOT_PLUG_OPTION)
                    bDVIDetect = 0;
#endif
	...
	case _ACTIVE_STATE:
		...
		{      
			#if (_HDMI_HOT_PLUG_OPTION == _ENABLE)			 //SHAN
