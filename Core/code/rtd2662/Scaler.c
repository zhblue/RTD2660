...
void CScalerInitial(void)
{
    ...
    CScalerSetBit(_HOST_CTRL_01, ~_BIT0, _BIT0);//	软件复位
    CTimerDelayXms(20);
    CScalerSetBit(_HOST_CTRL_01, ~_BIT0, 0x00);
    //CIrdaInitial();//红外设定
	CScalerCodeW(tSCALER_POWERUP_INITIAL);//寄存器值设定
...
void CScalerReset(void)
{
	...
	if(GET_POWERSTATUS())
	{
        ...
        if (_GET_INPUT_SOURCE() == _SOURCE_YPBPR)
        {
       	...
       	CAdjustGamma(_COMPACT_GAMMA_NORMAL_TABLE, tGAMMA_COMPACT1, tGAMMA_COMPACT1, tGAMMA_COMPACT1);//伽马值
