...
BYTE CloadMaxChannelNumber(void)
{
	...
	if (_GET_TV_TYPE() == _TV_CATV)	return (_CATV_MAX_CHANNEL-1);
	else							return (_AIR_MAX_CHANNEL-1);
#endif
...
{         
 ...
 //   CPowerLightPowerOff();
	...
	// force to background
    CAdjustBackgroundColor(0x00, 0x00, 0x00);     
	CScalerSetBit(_VDISP_CTRL_28, 0xf7, _BIT5);	// Display output is forced to the background color,and free run//设置为自动
    ucSearchCount  = 1;
    bChangeChannel = 1;
    bTVNoSignal    = 0;
   #endif
